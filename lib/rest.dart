...
class REST {
  ...
  REST([this.port = 80]) {
    ...
    print("[REST] REST Object created.");
    this.handleNotFound = this._onHttpHandleNotFound;
  }

  RouteObject _getMatchedObject(HttpRequest request) {
    print("[REST] HttpResquest: ${request.method} - [${request.uri.toString()}] (${request.requestedUri.host} on port ${request.requestedUri.port})");
  ...
  void run() {
    print("[REST] HttpServer listen on ${this.ip}:${this.port}");
